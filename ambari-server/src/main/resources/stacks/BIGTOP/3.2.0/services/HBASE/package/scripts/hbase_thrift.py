...
class HbaseThrift(Script):
  ...
  def configure(self, env):
    ...
    # Build effective hbase-site.xml for Thrift without mutating immutable Ambari config dicts.
  ...
  def start(self, env, upgrade_type=None):
    ...
    daemon_cmd = format(
      "{cmd} start {role} -p {hbase_thrift_port} --infoport {hbase_thrift_info_port}"
    )
